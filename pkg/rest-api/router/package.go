...
func ListPackage(c echo.Context) error {
	...
	return app.Send(c, http.StatusOK, pkgList)
}

func GetPackageAllVersions(c echo.Context) error {
	common.CBLog.Debugf("[CALLED]")

	if err := app.Validate(c, []string{"namespace", "package"}); err != nil {
		common.CBLog.Error(err)
		return app.SendMessage(c, http.StatusBadRequest, err.Error())
	}
	...
	return app.Send(c, http.StatusOK, pkg)
}

func GetPackage(c echo.Context) error {
	common.CBLog.Debugf("[CALLED]")

	if err := app.Validate(c, []string{"namespace", "package", "version"}); err != nil {
		common.CBLog.Error(err)
		return app.SendMessage(c, http.StatusBadRequest, err.Error())
	}
...
func UploadPackage(c echo.Context) error {
	...
	if err != nil {
		common.CBLog.Error(err)
		return app.Send(c, status.Code, status)
	}
